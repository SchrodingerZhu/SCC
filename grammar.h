...
#define DEFINE_KEYWORD(NAME, ...) \
    ...
    RULE(code_block, SpaceInterleaved<Char < '{'>, Asterisk<SpaceInterleaved < statement>>
    , Char<'}'>>)
    ...
    RULE(statement, Ord<SpaceInterleaved < Ord <
                        array_assign, var_assign, array_decl, var_decl, expression, return_statement, dowhile_statement, Nothing>, Char<';'>>
    , code_block, ifelse_statement, while_statement>)
    ...
    >)
    RULE(program, Seq<Start, Plus < SpaceInterleaved < toplevel>>
    , End>)
